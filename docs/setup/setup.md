# 1. Script de Setup (`setup.sh`)

Para facilitar a compilação e execução, inclua este script na raiz do repositório:
...
## Como usar o `setup.sh`

1. Copie este conteúdo para um arquivo chamado **`setup.sh`** na raiz do repositório.  
...
3. Defina a variável de ambiente `OLB_ROOT`:

   ```bash
   export OLB_ROOT="${HOME}/openlb"
   ```
...
4. Execute o script:
   ...
   Ele criará o build e o executável `cylinder2d` em `build/examples/forBeginners/`.
